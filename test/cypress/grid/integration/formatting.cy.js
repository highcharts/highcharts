describe('Formatting cells.', () => {
    ...
    it('Cells should be formatted.', () => {
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(0).should('contain', 'Jan 01, 2022');
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(2).should('contain', '100.0 kg');
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(3).should('contain', '$ 1.50');
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(4).within(() => {
            cy.get('a').should('contain', 'Apples URL');
        });

        cy.get('.highcharts-grid-row').eq(0).find('td').eq(2).should('have.attr', 'data-value', '100');
    });

    it('Cells without formatter should not be formatted.', () => {
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(1).should('contain', 'Apples');
    });

    it('CSS class and style should be applied.', () => {
        cy.get('.highcharts-grid-row').eq(0).find('td.custom-column-class-name')
            .should('have.css', 'color', 'rgb(255, 0, 0)');
    });

    it('The cell containing text should lose format when editing and gain back when not.', () => {
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(2).dblclick();
        cy.get('.highcharts-grid-edited-cell input').as('inputField');
        cy.get('@inputField').should('have.value', '100');
        cy.get('@inputField').clear().type('300');
        cy.get('body').click();
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(2).should('contain', '300.0 kg');
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(2).should('have.attr', 'data-value', '300');
    });

    it('The cell containing dates should lose format when editing and gain back when not.', () => {
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(0).dblclick();
        cy.get('.highcharts-grid-edited-cell input').as('inputField');
        cy.get('@inputField').should('have.value', '1640995200000');
        cy.get('@inputField').clear().type('1641081600000');
        cy.get('body').click();
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(0).should('contain', 'Jan 02, 2022');
    });
    ...
    it('The grid should adjust its width dynamically to the container width.', () => {
        ...
        cy.get('.highcharts-grid-table').should('exist').then(($el) => {
    ...
    it('Default formatter is not applied when column has own format or formatter', () => {
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(0).should('not.contain', 'Default');
        cy.get('.highcharts-grid-row').eq(0).find('td').eq(4).should('not.contain', 'Default');
    });
});
