describe('Cell class name formats.', () => {
    ...
    it('Table should have custom class name.', () => {
        cy.get('.highcharts-grid-table').should('have.class', 'custom-table-class-name');
        cy.get('.highcharts-grid-table').should('have.class', 'abc');
    });

    it('Custom class name should be refreshed on setValue.', () => {
        cy.get('.highcharts-grid-row[data-row-index="1"] > td[data-column-id="weight"]').as('cellA');
        cy.get('.highcharts-grid-row[data-row-index="3"] > td[data-column-id="weight"]').as('cellB');
