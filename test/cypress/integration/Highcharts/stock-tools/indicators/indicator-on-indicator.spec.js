describe('An indicator on indicator, #15696.', () => {
    ...
    it('There should be a possibility to add indicators based on other indicator, #15696.', () => {
        ...
        cy.openIndicators();
        cy.selectIndicator('SMA');
        cy.get('#highcharts-select-series').contains('SMA (14)');

        cy.get('#highcharts-select-series').select('SMA (14)');

        cy.get('input[name="highcharts-sma-period"]').eq(0).clear().type('20');
        ...
        cy.get('#highcharts-select-series').contains('SMA (20)');

        cy.get('.highcharts-tab-item').eq(1).click(); // Open EDIT bookmark.
