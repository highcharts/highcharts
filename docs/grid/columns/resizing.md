...
Column resizing is a global rendering concern and cannot be configured per column. Because resizing affects how widths are recalculated and redistributed across the entire table, it is not part of `columnDefaults` and cannot be overridden in `columns[]`. Instead, configure resizing under `rendering.columns.resizing`.
...
Once a column has been resized by the user, its width becomes **fixed in pixels**, regardless of whether it was originally defined as `'auto'`, a percentage, or left undefined.
