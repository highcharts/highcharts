...
## Grid Key vs License ID

The Grid Key is only used by Grid Pro through the `gridKey` configuration option. It is not the same as the License ID.

The License ID identifies your actual Highcharts license. Use the License ID when you need to identify your license, for example when contacting Highsoft support. The License ID is private and should not be shared publicly.

## How to set the Grid Key

You can set the Grid Key once for the whole page, or pass the Grid Key to specific Grid Pro instances. Only the Grid Key should be passed to `gridKey`; do not use the License ID there. The Grid Key can be obtained in your License Statement.
