...
Use `columns[].aggregator` to derive parent values from their direct
...
`columns[].treeView.aggregator` is deprecated since v3.1.0. Use
`columns[].aggregator`, which is shared with
