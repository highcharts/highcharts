...
See [live demo of using shadow options](https://jsfiddle.net/gh/get/library/pure/highcharts/samples/highcharts/plotoptions/series-shadow).
...
*   easing - Custom easing can be applied to all animation. See [Easing Functions](https://easings.net/) for a nice overview over some possible functions. Easing functions can either be passed directly, or registered on the `Math` object and referenced by name. See [chart/animation-easing](https://jsfiddle.net/gh/get/library/pure/highchartshighcharts/tree/main/t/animation-easing) for a live demo. The default easing is `easeInOutSine`.
