...
QUnit.test('Markers enabled, point marker off', function (assert) {
    ...
    assert.strictEqual(hasMarker(pointB), true, 'Point marker exists after ' +
        'update');
    assert.strictEqual(hasVisibleMarker(pointB), false, 'Point marker hidden ' +
        'after update');
});
...
QUnit.test(
    ...
    function (assert) {
        ...
        assert.strictEqual(hasMarker(point), false, 'Markers does not exist ' +
            'from start');

        chart.series[0].removePoint(0);
        point = chart.series[0].points[0];

        assert.strictEqual(!chart.boosted, true, 'Series should not be ' +
            'boosted anymore');
        assert.strictEqual(hasMarker(point), true, 'Markers should exist ' +
            'when series is not boosted');

        chart.series[0].addPoint(1);
        point = chart.series[0].points[0];

        assert.strictEqual(chart.boosted, true, 'Series should be boosted ' +
            'again');
        assert.strictEqual(hasMarker(point), false, 'Markers should not ' +
            'exist again');
