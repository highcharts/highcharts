QUnit.test('Show loading', function (assert) {
    ...
    try {
        ...
        assert.equal(label.innerHTML, 'Loading AJAX...', 'First text');

        chart.showLoading('Loading image...');

        assert.equal(label.innerHTML, 'Loading image...', 'Second text');

        chart.showLoading();

        assert.equal(label.innerHTML, 'Loading...', 'Default text');
