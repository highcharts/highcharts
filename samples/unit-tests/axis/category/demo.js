...
QUnit.test('Keeping names updated with dynamic data', function (assert) {
    ...
    assert.strictEqual(names.toString(), 'Oranges,Bananas', 'Series.remove');
    ...
    assert.strictEqual(names.toString(), 'Upda2,Addid1,Addid2', 'Point.remove');
});
...
QUnit.test('Extremes unaltered after redraw (#5928)', function (assert) {
    ...
    assert.strictEqual(chart.xAxis[0].max, 2, 'Initial max');

    chart.setSize(390);

    assert.strictEqual(chart.xAxis[0].min, 0, 'Unaltered min');

    assert.strictEqual(chart.xAxis[0].max, 2, 'Unaltered max');

    chart.series[0].addPoint(['2016', 3]);

    assert.strictEqual(chart.xAxis[0].min, 0, 'Unaltered min');

    assert.strictEqual(chart.xAxis[0].max, 3, 'Increased max');
});
...
QUnit.test(
    ...
    function (assert) {
        ...
        assert.strictEqual(chart.xAxis[0].min, 0, 'Correct min');
        assert.strictEqual(chart.xAxis[0].max, 11, 'Correct max');
    }
);
...
QUnit.test(
    ...
    function (assert) {
        ...
        assert.ok(
            yAxes[0].categories && yAxes[0].options.showLastLabel,
            'First axis has categories and shows last label'
        );

        assert.notOk(
            yAxes[1].categories || yAxes[1].options.showLastLabel,
            '2nd axis does not have categories or shows last label'
        );

        assert.ok(
            yAxes[2].categories && yAxes[2].options.showLastLabel,
