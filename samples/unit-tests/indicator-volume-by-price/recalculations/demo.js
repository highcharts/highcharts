QUnit.test('Test algorithm on data updates.', function (assert) {
    var chart = Highcharts.stockChart('container', {
        ...
        series: [
            ...
            },
            {
                ...
                yAxis: 1,
                data: [
                    109643597, 96940319, 97194965, 102037313, 107341570,
                    163025387, 158618691, 164822833, 167418881, 146353774,
                    196528871, 162371804, 120774913, 258760411, 117410608,
                    168347452, 112101444, 108824968, 125491450, 167716514,
                    102099277, 164600814, 106937733, 139634663, 157522589,
                    108823925, 230548444, 276114391, 308189833, 180406030,
                    137864321
                ]
            },
            {
    ...
    var expectedData = [
        303778881, 209378883, 321644078, 108824968, 612690610, 1114360905,
        447747531, 246572396, 266346514, 0, 626460184, 506662835
    ];
    ...
    assert.deepEqual(
        round(indicator.volumeDataArray),
        [
            303778881, 209378883, 321644078, 970044968, 612690610, 1114360905,
            447747531, 246572396, 266346514, 0, 626460184, 506662835
        ],
    ...
    assert.deepEqual(
        round(indicator.volumeDataArray),
        [
            303778881, 209378883, 321644078, 841324968, 612690610, 1114360905,
            447747531, 246572396, 266346514, 0, 626460184, 506662835
        ],
