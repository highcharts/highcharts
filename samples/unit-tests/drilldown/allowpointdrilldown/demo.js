QUnit.test('Drill down on single point is not allowed', function (assert) {
    ...
    assert.equal(chart.series.length, 2, '2 series on first level');
    assert.equal(chart.series[0].name, '2010', 'Check first series');

    // Click first point
    Highcharts.fireEvent(chart.series[0].points[0], 'click');
    assert.equal(chart.series.length, 2, '2 series on second level');
    assert.equal(chart.series[0].name, 'Republican 2010', 'Check first series');

    // ... and, we're back
    chart.drillUp();
    assert.equal(chart.series[0].name, '2010', 'First level name');
...
QUnit.test('Drilling with keys', function (assert) {
    ...
    var chart = Highcharts.chart('container', {
        ...
        series: [
            ...
            },
            {
                name: '2014',
                data: [
                    ['Republican', 4, 'republican-2014'],
                    ['Democrats', 4, 'democrats-2014'],
                    ['Other', 4, 'other-2014']
                ],
                keys: ['name', 'y', 'drilldown']
            }
    ...
    assert.strictEqual(chart.series.length, 2, 'Both series should be drilled');
});
