...
QUnit.test('Wrong datalabel position (#3648)', function (assert) {
    ...
    var pointsFirstSeries = chart.series[0].points,
        ...
        heightOfFirstSeries = chart.container.childNodes[0].childNodes[4].getBBox().height,
        xPosForDataLabels = [],
        columnHeights = [];
    ...
    for (var j = 0; j < pointsSecondSeries.length; j++) {
        columnHeights.push(heightOfFirstSeries + pointsSecondSeries[j].graphic.getBBox().height);
        xPosForDataLabels.push(heightOfFirstSeries + pointsSecondSeries[j].dataLabel.x);
    }
