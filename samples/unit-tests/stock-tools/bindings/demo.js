...
QUnit.test(
    ...
    (assert) => {
        ...
        assert.notOk(coordsX, 'No xAxis should be found.');

        assert.notOk(coordsY, 'No yAxis should be found.');
        ...
        assert.ok(coordsX, 'The xAxis should be found.');
...
QUnit.test(
    "Stock Tools annotations' positions with yAxis.top (#15075)",
    (assert) => {
        const chart = Highcharts.stockChart('container', {
                chart: {
                    width: 800
        ...
        assert.strictEqual(
            ...
            "Annotation's element should be placed " +
                yOffset +
