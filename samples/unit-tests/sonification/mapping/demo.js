QUnit.module('Mapping tests', hooks => {
    ...
    QUnit.test('Basic mappings', function (assert) {
        ...
        assert.strictEqual(channels.length, 2, 'Has two channels, one for ' +
            'each track');
        assert.strictEqual(channels[0].type,
            'instrument', 'First track is instrument');
        assert.strictEqual(channels[1].type,
            'speech', 'Second track is speech');

        function assertInstrumentEvent(event, expectedTime, expectedProps) {
            assert.strictEqual(event.time, expectedTime, 'Instrument event ' +
                'has expected time');
            assert.propEqual(event.instrumentEventOptions, expectedProps,
                'Instrument event has expected props');
        }

        function assertSpeechEvent(event, expectedTime, text, expectedProps) {
            assert.strictEqual(event.time, expectedTime, 'Speech event has ' +
                'expected time');
            assert.strictEqual(event.message, text, 'Speech event has ' +
                'expected text');
            assert.propEqual(event.speechOptions, expectedProps, 'Speech ' +
    ...
    QUnit.test('Mapping functions', function (assert) {
        ...
        assert.strictEqual(
            sonification.timeline.channels[2].events
                .map(e => Math.round(
                    e.instrumentEventOptions.pan * 1000) / 1000
                )
                .join(','),
