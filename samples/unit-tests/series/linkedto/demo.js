    ...
    $('#container').highcharts({
        series: [
            ...
            },
            {
                name: 'Tokyo Bis',
                data: [
                    -0.2, 0.8, 5.7, 11.3, 17.0, 22.0, 24.8, 24.1, 20.1, 14.1,
                    8.6, 2.5
                ],
                linkedTo: 'tokyo',
                visible: false
...
QUnit.test(
    'Series should inherit visibility from parent when is linked.(#3879)',
    function (assert) {
        var chart = $('#container')
            .highcharts({
                series: [
...
QUnit.test('Linked series update (#3028)', function (assert) {
    TestTemplate.test(
        ...
        function (template) {
            ...
            for (i = 0, ie = series.length; i < ie; ++i) {
                assert.ok(!series[i].visible, 'Series should not be visible.');
            }
        }
    );
});
