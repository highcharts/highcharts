(async () => {
    ...
    QUnit.test('General dataGrouping options', function (assert) {
        ...
        assert.notOk(
            calledWithNaN,
            'Empty series should not cause getTimezoneOffset to get called ' +
            'with NaN timestamp (#13247)'
        );
    ...
    QUnit.test('dataGrouping approximations', function (assert) {
        ...
        assert.strictEqual(
            ...
            true,
            '"range" approximation should return nulls when all points in a ' +
            'group are nulls (#6716).'
        );
    ...
    QUnit.test('dataGrouping and multiple series', function (assert) {
        ...
        assert.ok(
            ...
            'Points array is nullified for a hidden series. Hidden series ' +
            'shouldn\'t have `undefined`-points in a series.points array ' +
            '(#6709).'
        );
    });
    ...
    QUnit.test('Switch from non-grouped to grouped', function (assert) {
        ...
        assert.strictEqual(
            ...
            0,
            'After series update no old markers should be left on the chart ' +
            '(#10745)'
        );
    });
    ...
    QUnit.test('Data grouping and extremes change', function (assert) {
        ...
        assert.strictEqual(
            ...
            true,
            'minPadding should decrease xAxis.min even when points are ' +
            'grouped (#10932).'
        );
    ...
    QUnit.test(
        'DataGrouping with selected range and update', function (assert) {
    ...
    QUnit.test(
        ...
        function (assert) {
            ...
            // Call function responsible for calculating positions of invisible
            // points while panning.
    ...
    QUnit.test('The dataGrouping enabling/disabling.', function (assert) {
        ...
        // When the dataGrouping gets off, the properties should be deleted,
        // #16238.
        mapArray.forEach(prop => {
            assert.notOk(
                series[prop],
    ...
    QUnit.test(
        'Data grouping multiple series on zoom, #17141.', function (assert) {
            const chart = Highcharts.stockChart('container', {
                chart: {
                    width: 500
                },
