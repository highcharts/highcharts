...
QUnit.test('Navigator updates and redraws', function (assert) {
    let redrawsAmount = 0;
    const chart = Highcharts.stockChart('container', {
        chart: {
            type: 'line',
    ...
    assert.strictEqual(
        chart.navigator.handles[0].symbols[0],
        'circle',
        'Navigator handles should be updated to new symbol.'
    );
    assert.strictEqual(
        chart.navigator.handles[0].height,
        30,
        'Navigator handles should be updated to new height.'
    );
    assert.strictEqual(
        chart.navigator.handles[0].width,
        30,
