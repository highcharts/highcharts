QUnit.test('Timeline: General tests.', function (assert) {
    ...
    assert.strictEqual(
        series.visiblePointsCount === 3 && !series.points[2].visible,
        true,
    ...
    assert.strictEqual(
        series.visiblePointsCount === 3 && series.points.length === 3,
        true,
...
QUnit.test('Timeline: Styled mode #17614', function (assert) {
    ...
    series.points.forEach(point => {
        const colorIndex = point.colorIndex;

        assert.ok(
            point.connector
                .hasClass(`highcharts-color-${colorIndex}`),
            'Connector has class with the same colorIndex as the point'
        );

        assert.ok(
            point.dataLabel.element.innerHTML.includes(
                '<tspan class="highcharts-color-' + colorIndex +
                '">● </tspan>'),
            'Datalabel dot has class with the same colorIndex as the point'
        );
    });
