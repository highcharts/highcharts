let itemChart;
	...
	itemChart = Highcharts.chart('container', {
		...
		chart: {
			...
			events:{
				load:function(){
				itemChart = this;
				chartToChange = itemChart.series.findIndex(series => series.options.type=='item');

				$('#item1').attr('value',chartToChange);
				$('#item2').attr('value',chartToChange+1);


				nGraph = itemChart.series.findIndex(series => series.options.type=='networkgraph');
			...
			scatter:{ 
				dataLabels:{
	      		 	...
	      		 	formatter:function(){
	      		 		...
	      		 		return '<div class="message" style="background-color:#fff;color:'+ msgColors[index][0] + '">'+name+'</div>'
	      		 	}
	      		},
	      		marker:{
	      			enabled:false,
	...
	soundChart = Highcharts.chart('soundContainer', {
		chart: {
			type: 'scatter'
		},
		title: {
		...
		tooltip: {
			...
			pointFormatter: function(){
				let pointDate = new Date(this.options.x);
...
document.getElementById('play').onclick = function () {
	change1();
  if (!soundChart.sonification.timeline || soundChart.sonification.timeline.atStart()) {
   soundChart.sonify({
...
function change1(){
	...
	setTimeout(function(){
		...
		ski(direction);
		let santaSki = setInterval(function(){
		...
		,7500);
		$('g.highcharts-point.highcharts-color-1[aria-label="12. sun disk, 1. Christmas Tree."]').css({'transform':'translate(30px, -20px)'});
	},2000);
...
function ski(direction){
	...
	if(direction=='forward'){
		...
		ski1();
		setTimeout(function(){ski2()},1000);
		setTimeout(function(){ski3()},2000);
		setTimeout(function(){ski4()},3000);
		setTimeout(function(){ski5()},5000);
		setTimeout(function(){ski6()},6000);
	}else{
		itemChart.series[5].points[0].update({
			marker:{
		...
		ski5();
		setTimeout(function(){ski4()},1000);
		setTimeout(function(){ski3()},3000);
		setTimeout(function(){ski2()},4000);
		setTimeout(function(){ski1()},5000);
	}
}
