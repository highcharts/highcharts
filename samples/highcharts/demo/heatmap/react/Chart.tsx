import Highcharts from 'highcharts/es-modules/masters/highcharts.src.js';
import {
    Chart,
    Title,
    Subtitle,
    XAxis,
    YAxis,
    Legend,
    Tooltip
} from '@highcharts/react';
import { HeatmapSeries } from '@highcharts/react/series/Heatmap';
import type { HeatmapSeriesProps } from '@highcharts/react/series/Heatmap';
import { Exporting } from '@highcharts/react/modules/Exporting';
import { Accessibility } from '@highcharts/react/modules/Accessibility';
import 'highcharts/es-modules/masters/modules/heatmap.src.js';

// Substring template helper for the responsive labels
Highcharts.Templating.helpers.substr = (
    value: unknown,
    from: unknown,
    length: unknown
): string => String(value).substr(Number(from), Number(length));

const data: NonNullable<HeatmapSeriesProps['data']> = [
    [0, 0, 10],
    [0, 1, 19],
    [0, 2, 8],
    [0, 3, 24],
    [0, 4, 67],
    [1, 0, 92],
    [1, 1, 58],
    [1, 2, 78],
    [1, 3, 117],
    [1, 4, 48],
    [2, 0, 35],
    [2, 1, 15],
    [2, 2, 123],
    [2, 3, 64],
    [2, 4, 52],
    [3, 0, 72],
    [3, 1, 132],
    [3, 2, 114],
    [3, 3, 19],
    [3, 4, 16],
    [4, 0, 38],
    [4, 1, 5],
    [4, 2, 8],
    [4, 3, 117],
    [4, 4, 115],
    [5, 0, 88],
    [5, 1, 32],
    [5, 2, 12],
    [5, 3, 6],
    [5, 4, 120],
    [6, 0, 13],
    [6, 1, 44],
    [6, 2, 88],
    [6, 3, 98],
    [6, 4, 96],
    [7, 0, 31],
    [7, 1, 1],
    [7, 2, 82],
    [7, 3, 32],
    [7, 4, 30],
    [8, 0, 85],
    [8, 1, 97],
    [8, 2, 123],
    [8, 3, 64],
    [8, 4, 84],
    [9, 0, 47],
    [9, 1, 114],
    [9, 2, 31],
    [9, 3, 48],
    [9, 4, 91]
];

export default function HeatmapChart() {
    return (
        <Chart
            options={{
                chart: {
                    plotBorderColor: '#0443E1',
                    plotBorderWidth: 2,
                    plotBorderRadius: 5,
                    marginBottom: 25
                },
                colorAxis: {
                    min: -4,
                    max: 154,
                    startOnTick: false,
                    endOnTick: false,
                    tickPixelInterval: 50,
                    minColor: '#0443E111',
                    maxColor: '#0443E1',
                    // Dots for ticks, extended also by custom CSS
                    tickColor: '#ffffff',
                    tickLength: 0.1,
                    tickWidth: 6,
                    gridLineWidth: 0,
                    marker: {
                        symbol: 'circle',
                        color: 'transparent',
                        lineColor: '#fff',
                        lineWidth: 3
                    }
                },
                responsive: {
                    rules: [
                        {
                            condition: { maxWidth: 500 },
                            chartOptions: {
                                yAxis: {
                                    labels: {
                                        rotation: 0,
                                        format: '{substr value 0 1}'
                                    }
                                }
                            }
                        }
                    ]
                }
            }}
        >
            <Title align="left">Simple heatmap</Title>
            <Subtitle align="left">Sales per employee per weekday</Subtitle>
            <XAxis
                categories={[
                    'Alexander',
                    'Marie',
                    'Maximilian',
                    'Sophia',
                    'Lukas',
                    'Maria',
                    'Leon',
                    'Anna',
                    'Tim',
                    'Laura'
                ]}
                opposite={true}
                labels={{
                    autoRotation: [0, -90],
                    style: { fontSize: '0.65em' }
                }}
                lineWidth={1}
                lineColor="#0443E1"
            />
            <YAxis
                categories={[
                    'Monday',
                    'Tuesday',
                    'Wednesday',
                    'Thursday',
                    'Friday'
                ]}
                title={{ text: null }}
                reversed={true}
                labels={{
                    rotation: -90,
                    style: { fontSize: '0.65em' }
                }}
            />
            <Accessibility
                point={{
                    descriptionFormat:
                        '{(add index 1)}. ' +
                        '{series.xAxis.categories.(x)} sales ' +
                        '{series.yAxis.categories.(y)}, {value}.'
                }}
            />
            <Legend
                align="right"
                layout="vertical"
                symbolRadius={6}
                margin={0}
                verticalAlign="bottom"
                symbolHeight={270}
            />
            <Tooltip padding={12}>
                {`<div
                style="border-left: 3px solid {point.color}; padding-left: 8px;"
                >
                <b>{series.xAxis.categories.(point.x)}</b> sold
                <b>{point.value}</b><br>
                items on <b>{series.yAxis.categories.(point.y)}</b></div>`}
            </Tooltip>
            <HeatmapSeries
                name="Sales per employee"
                data={data}
                options={{
                    borderColor: '#0443E1',
                    borderWidth: 1,
                    dataLabels: {
                        enabled: true,
                        style: { fontSize: '0.85em' }
                    }
                }}
            />
            <Exporting />
        </Chart>
    );
}
