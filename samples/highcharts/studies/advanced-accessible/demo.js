$(function () {
    ...
    H.Chart.prototype.callbacks.push(function (chart) {
        ...
        if (chartDesc.slice(-1) !== '.') chartDesc += '.';
...
//*/
        ...
        H.addEvent(chart.renderTo, 'keydown', function (e) {
            ...
            if (!chart.isExporting) {
                switch (e.which) {
                    ...
                    case 39: // Right
                        if (!chart.highlightAdjacentPoint(e.which === 39, !doExporting && wrap)) {
                            if (doExporting && wrap) {
                                console.log("Should start exporting");
                ...
                switch (e.which) {
                    ...
                    case 40: // Down
                        ...
                        reachedEnd = true;
                        for (var i = highlightedExportItem + 1; i < exportList.length; ++i) {
                            if (exportList[i] && exportList[i].tagName === 'DIV'
                                    && !(exportList[i].children && exportList[i].children.length)) {
                                exportList[i].focus();
                                exportList[chart.highlightedExportItem].onmouseout();
