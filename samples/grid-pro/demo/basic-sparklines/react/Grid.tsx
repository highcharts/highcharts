...
import { CellRendererRegistry, DataTable, type TableCell } from '@highcharts/grid-pro';
import { Grid, type GridOptions, type GridRefHandle } from '@highcharts/grid-pro-react';

CellRendererRegistry.types.sparkline.useHighcharts(Highcharts);
...
export default function GridComponent() {
    const [options] = useState<GridOptions>({
        ...
        columns: [{
            ...
            cells: {
                renderer: {
                    type: 'sparkline',
                    chartOptions: function (data: number) {
                        return {
                            chart: {
                                type: 'pie',
                                height: 30
            ...
            cells: {
                renderer: {
                    type: 'sparkline',
                    chartOptions: function (this: TableCell, data: string) {
    ...
    useEffect(() => {
        ...
        function scheduleUpdate(rowIndex: number) {
        ...
        function generateArrayFlow(stringArray: string) {
        ...
        // Function to update the instance status in the data table and refresh
        // the cells. It updates the data even if the cells are not rendered in
        // the grid.
        async function updateInstanceStatus(rowIndex: number) {
            const grid = gridRef.current?.grid;
            if (!grid) {
                return;
            }

            const live = dataTable.getCell('live', rowIndex);
            if (!live) {
                return;
            }

            const oldRam = dataTable.getCell('ram', rowIndex);
            const oldCpu = dataTable.getCell('cpu', rowIndex);

            // Data Table cells can be updated directly, even if the cells are
            ...
            // row.loadData() is used to fetch the data from the data table into
            // the `row.data` object, because unlike the `column.data`, the
            // `row.data` is not a direct reference to the data table, but a copy
            // of the data for the row.
            row.loadData();
    ...
    return <Grid options={options} gridRef={gridRef} />;
}
