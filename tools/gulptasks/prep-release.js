...
function prepareRelease() {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (argv.cleanup) {
            ...
            if (argv.commit) {
                if (stagedChanges) {
                    reject(
                        new Error(
                            'You have other staged changes. Please commit or remove them first.'
                        )
                    );
                    return;
                }
                ChildProcess.execSync(
                    `git add build-properties.json && git commit -m"Cleaned up after v${packageJsonVersion}."`
                );
                LogLib.message(
                    'Clean up commit added. git push when you are ready.'
                );
            }
        ...
        LogLib.message(
            'Preparing version number update. Current version is ' +
                packageJsonVersion
        );
        ...
        if (!argv.cleanup && !nextVersion) {
            reject(
                new Error(
                    'Please provide either --nextversion x.y.z or --cleanup when starting the command.'
                )
            );
            return;
        }
        ...
        */
        ChildProcess.execSync(
            `sed -i'.bak' -e '/version/s/"${packageJsonVersion}"/'\\"${nextVersion}\\"/ package.json`
        );
        ChildProcess.execSync(
            `sed -i'.bak' -e '/version/s/"${bowerJsonVersion}"/'\\"${nextVersion}\\"/ bower.json`
        );
        ...
        const isGNU =
            ChildProcess.execSync(
                'xargs --version 2>&1 |grep -s GNU >/dev/null && echo true || echo false'
            )
                .toString()
                .replace('\n', '') === 'true';
        ChildProcess.execSync(
            `grep -Rl --exclude=*.bak --exclude-dir=node_modules --exclude-dir=code "@since\\s\\+next" . | xargs ${
                isGNU ? '-r' : ''
            } sed -i'.bak' -e 's/@since *next/@since ${nextVersion}/'`
        );

        LogLib.success(
            'Updated version in package.json, bower.json, build-properties.json and replaced @ since next' +
...
prepareRelease.description =
    'Prepares a new release by replacing version numbers with the supplied version. ' +
...
prepareRelease.flags = {
    '--cleanup':
        'Will add -modified to version and remove date from build-properties.json. Exludes --version',
    '--commit': 'Commit cleanup changes. Implies --cleanup.',
    '--nextversion':
        'Version that will replace version in package.json, build-properties.json, bower.json and "since next" in the docs.'
};
