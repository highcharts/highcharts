...
function uploadAPIDocs() {
    ...
    const { asyncForeach, uploadFiles } = require('../../upload.js');
    const argv = require('yargs').argv;
    const sourceFolder = './build/api/';
    const bucket = argv.bucket || process.env.HIGHCHARTS_APIDOCS_BUCKET;
    const batchSize = 30000;
    const files = isString(argv.files)
        ? argv.files.split(',')
        : getFilesInFolder(sourceFolder, true, '');

    if (!bucket && !argv.test) {
        throw new Error(
            'No --bucket argument specified or env. variable HIGHCHARTS_APIDOCS_BUCKET is empty or unset.'
        );
    }
    ...
    console.log(
        `Started upload of ${
            files.length
        } files to ${bucket} under tags [${tags.join(', ')}].`
    );
    const commands = [];
    return asyncForeach(tags, (tag) =>
        Promise.resolve(getUploadConfig(tag))
            .then(argv.test ? uploadFilesTest : uploadFiles)
            .then((result) => {
                const { errors } = result;
                if (errors.length) {
                    const erroredFiles = errors.map((e) =>
                        Path.relative(sourceFolder, e.from)
                            // Make path command line friendly.
                            .split(Path.sep)
                            .join('/')
                    );
                    commands.push(
                        `gulp upload-api --tags ${tag} --files ${erroredFiles.join(
                            ','
                        )}`
                    );
                }
...
uploadAPIDocs.flags = {
    ...
    '--files':
        'Upload selected files relative to current working directory. (optional)',
    '--profile':
    ...
    '--noextensions':
        'Remove file extensions for uploaded files (destination). Useful for testing/serving directly from S3 bucket (optional)',
    '--silent': "Don't produce progress output. (optional)",
    '--test':
        'Will save processed files for S3 bucket in a local directory instead of uploading. (optional)'
};
