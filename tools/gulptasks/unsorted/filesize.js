...
const filesize = async () => {
    ...
    const { getFile } = require('highcharts-assembler/src/utilities.js');
    ...
    const printRow = (sizes, content) =>
        content.map((c, i) => padRight(c.toString(), sizes[i])).join('');
    const report = (name, current, head) => {
        ...
        console.log(
            [
                ...
                printRow(colsizes, [
                    'Diff:',
                    diff(current.gzip, head.gzip) + 'B',
                    diff(current.compiled, head.compiled) + 'B',
                    diff(current.size, head.size) + 'B'
                ]),
                ''
            ].join('\n')
        );
