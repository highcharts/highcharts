...
function configureVisualTestRun() {
    log.starting(
        'Setting config for creating and uploading reference images..'
    );
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!tests) {
            const errMsg =
                'Please provide --tests using a comma separated list of path(s) or' +
                ' glob style like highcharts/3d/*.';
            log.failure(errMsg);
            reject(errMsg);
        }
        const forcedArgs = ['--reference', 'true', '--saveresetdate', 'true'];
...
gulp.task(
    ...
    gulp.series(configureVisualTestRun, 'test', 'dist-testresults')
);
