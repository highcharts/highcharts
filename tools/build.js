...
const { exists, getFile } = require('highcharts-assembler/src/utilities.js');
const { checkDependency } = require('./filesystem.js');
...
const getBuildOptions = (input) => {
    ...
    const files = isArray(input.files)
        ? input.files
...
const getExcludedFilenames = (requires, base) =>
    requires.reduce((arr, name) => {
        const filePath = join(
            base,
...
const watchESModules = (event, options, type, dependencies, pathESMasters) => {
    ...
    const filesModified = Object.keys(dependencies).reduce(
        (arr, pathMaster) => {
            const list = dependencies[pathMaster];
            if (list.includes(pathFile)) {
                arr.push(
                    relative(pathESMasters, pathMaster).split(sep).join('/')
                );
            }
            return arr;
        },
    ...
    const { debug, fileOptions, version } = options;
...
const getBuildScripts = (params) => {
    checkDependency('highcharts-assembler', 'warn', 'devDependencies');
    const options = getBuildOptions(params);
    const { files, type: types, mapTypeToSource } = options;
    const result = {
    ...
    types.forEach((type) => {
        ...
        const fn = (event) => {
            const dependencies = getListOfDependencies(files, pathSource);
