...
2. **Source checks:** Disable Compile on Demand in the utils UI and set `compileOnDemand` and `useMinifiedCode` to `false` in its `config.json`. Verify `highchartsDir` points to this checkout and restart utils. These checks also run when resuming the dry-run phase. Build Highcharts with `npx gulp scripts`, confirm the version in `tools/gulptasks/dashboards/build-properties.json`, then run `npm run gcode`, `npm run dtest`, `npx gulp test-cypress --product Dashboards` and `npm test`.
