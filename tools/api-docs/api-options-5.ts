...
function addTreeNode(
    ...
    for (const _moreInfo of _moreInfos) {
        ...
        addTreeNode(
            TSLib.getSourceInfo(_moreInfo.meta.file),
            _treeNode,
            _moreInfo
        );
...
async function main() {
    ...
    _timer = LogLib.starting(`Loading ${source}`);

    const _sources = TSLib.getProjectInfos(source);

    LogLib.finished(`Loading ${source}`, _timer);

    _timer = LogLib.starting(`Building tree from ${source}`);

    for (const _source of _sources) {
        addDefaultOptions(_source, debug);
    }
    LogLib.finished(`Building tree from ${source}`, _timer);
