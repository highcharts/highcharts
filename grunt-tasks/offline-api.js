/* eslint-env node */
/* eslint no-console: 0, no-invalid-this: 0, valid-jsdoc: 0 */
(function () {
    'use strict';
    module.exports = function (grunt) {
        ...
        var domain = 'http://api.highcharts.com',
            apiUrl = domain + '/',
        ...
        grunt.registerTask('download-api', 'Download the Highcharts API.', function (product, version) {
            ...
            function downloadAssets(assets) {
                 ...
                 * Download to folder
                 */
                function downloadToFolder(assetsForType, assetType) {
                    return function () {
                        var filesidx,
                            origin,
                            filename;
                        ...
                        for (filesidx = 0; filesidx < assetsForType.length; filesidx++) {
                            ...
                            download(origin, determineLocalPath(origin, assetType));
                        }
            ...
            function modifyPage(filename) {
                fs.readFile(filename, function (err, data) {
                    ...
                    fs.writeFile(filename, html, function (writeErr) {
                        if (writeErr) {
                            throw writeErr;
                        }
                    });
