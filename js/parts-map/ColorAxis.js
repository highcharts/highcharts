...
var addEvent = U.addEvent, erase = U.erase, extend = U.extend, fireEvent = U.fireEvent, Fx = U.Fx, isNumber = U.isNumber, merge = U.merge, pick = U.pick, splat = U.splat;
/**
 * Color axis types
 *
...
var ColorAxis = /** @class */ (function (_super) {
     ...
     * @return {boolean}
     */
    ColorAxis.prototype.shouldBeRendered = function () {
        return !!(this.options && this.options.showInLegend);
    };
    /**
     * Render color axis in legend.
     * @private
     * @function Highcharts.ColorAxis#renderAsLegendItem
     */
    ColorAxis.prototype.renderAsLegendItem = function () {
        var item = this;
        if (!item.legendGroup) {
            item.legendGroup = item.chart.renderer
                .g('legend-item')
                .addClass('highcharts-coloraxis')
                .attr({
                zIndex: 1
            })
                .add(item.legend.scrollGroup);
        }
        item.legend.baseline = 15;
        // Always create a new symbol.
        if (item.legendSymbol) {
            item.legendSymbol.destroy();
        }
        item.drawLegendSymbol(item.legend, this);
        item.itemWidth = item.legendItemWidth;
        item.itemHeight = item.legendItemHeight;
        item.legend.maxItemWidth =
            Math.max(item.legend.maxItemWidth, item.itemWidth);
        item.legend.totalItemWidth += item.itemWidth;
        item.legend.itemHeight = item.itemHeight;
        // Fake legend item is required for now -
        // for color axis only legend symbol is used.
        item.legendItem = item.chart.renderer.text('', 0, 0);
        if (item.visible && item.legendColor && item.legendSymbol) {
