...
HR.prototype.arc3d = function (shapeArgs) {
	...
	result.attrSetters['stroke-width'] = function(i) {
		// Force all to 0		
		return 0;
	}
	
	result.animate = function (arguments) {
		
		if (arguments.translateX !== undefined && arguments.translateX !== undefined )
		{
			this.translate(arguments.translateX, arguments.translateY);
...
HR.prototype.arc3dPath = function (shapeArgs) {
    ...
    if (arcAngle > PI / 2) {        
        var p1 = this.arc3dPath({x: x, y: y, depth: d, start: start, end: start + (PI / 2), r: r, innerR: ir, alpha: alpha, beta: beta});
        var p2 = this.arc3dPath({x: x, y: y, depth: d, start: start + (PI / 2) + 0.00001, end: end, r: r, innerR: ir, alpha: alpha, beta: beta});
		...
		if (start < 0 && end > 0) {       
            var p1 = this.arc3dPath({x: x, y: y, depth: d, start: start, end: 0, r: r, innerR: ir, alpha: alpha, beta: beta});
            var p2 = this.arc3dPath({x: x, y: y, depth: d, start: 0, end: end, r: r, innerR: ir, alpha: alpha, beta: beta});
            ...
            var p1 = this.arc3dPath({x: x, y: y, depth: d, start: start, end: PI, r: r, innerR: ir, alpha: alpha, beta: beta});
            var p2 = this.arc3dPath({x: x, y: y, depth: d, start: -PI, end: end, r: r, innerR: ir, alpha: alpha, beta: beta});
