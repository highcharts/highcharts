...
var GridAxisAdditions = /** @class */ (function () {
    ...
    GridAxisAdditions.prototype.isOuterAxis = function () {
        var grid = this;
        var axis = grid.axis, chart = axis.chart, columnIndex = grid.columnIndex, columns = (axis.linkedParent &&
            axis.linkedParent.grid &&
            axis.linkedParent.grid.columns ||
            grid.columns), parentAxis = columnIndex ? axis.linkedParent : axis, thisIndex = -1, lastIndex = 0;
...
var GridAxis = /** @class */ (function () {
    ...
    GridAxis.onAfterGetOffset = function () {
        (this.grid && this.grid.columns || []).forEach(function (column) {
            column.getOffset();
        });
    };
    /**
    ...
    GridAxis.onAfterInit = function () {
        var axis = this, chart = axis.chart, grid = axis.grid, userOptions = axis.userOptions, options = axis.options, gridOptions = options.grid || {};
        if (gridOptions.enabled) {
            applyGridOptions(axis);
            /* eslint-disable no-invalid-this */
            // TODO: wrap the axis instead
            wrap(axis, 'labelFormatter', function (proceed) {
                var axis = this.axis, tickPos = axis.tickPositions, value = this.value, series = (axis.isLinked && axis.linkedParent ?
                    axis.linkedParent :
        ...
        if (grid && gridOptions.columns) {
            var columns = grid.columns = [], columnIndex = grid.columnIndex = 0;
    ...
    GridAxis.onAfterSetAxisTranslation = function () {
        var axis = this, options = axis.options, userOptions = axis.userOptions, gridOptions = options.grid || {}, tickInfo = this.tickPositions && this.tickPositions.info, userLabels = userOptions && userOptions.labels || {};
        if (this.horiz) {
            if (gridOptions.enabled === true) {
    ...
    GridAxis.onAfterSetOptions = function (e) {
        ...
        var gridOptions = options && options.grid || {};
        var userGridOptions = userOptions && userOptions.grid || {};
        var columns = userGridOptions.columns;
        var gridAxisOptions;
        if (gridOptions.enabled === true) {
