...
if [ $DOCS_COMMIT = $LATEST_COMMIT ]; then
    ...
    # Circle API v2
    curl -X POST -d '{ "parameters": { "run_deploy": true, "target_bucket": "${BUCKET}" }}' \
    https://circleci.com/api/v2/project/github/highcharts/doc-builder/pipeline?circle-token=${TOKEN}
else
