...
test.describe('Grid Lite row pinning', () => {
    ...
    test('Pinned sections have no internal max-height scrolling by default', async ({ page }) => {
        const state = await page.evaluate(async () => {
            ...
            return {
                ...
                overflowX: topTbody.style.overflowX,
        ...
        expect(state.overflowY).toBe('');
        expect(state.overflowX).toBe('');
    ...
    test('Pinned and main sections stay right-edge aligned', async ({ page }) => {
        const state = await page.evaluate(async () => {
            const grid = (window as any).grid;
