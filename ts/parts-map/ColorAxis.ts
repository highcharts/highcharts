...
class ColorAxis extends Axis implements AxisLike {
    ...
    public renderAsLegendItem(): void {
        const item = this as unknown as (ColorAxis&Highcharts.LegendItemObject);
        if (!item.legendGroup) {
        ...
        item.legend.maxItemWidth =
            Math.max(item.legend.maxItemWidth, item.itemWidth as number);

        item.legend.totalItemWidth += item.itemWidth as number;
        item.legend.itemHeight = item.itemHeight as number;

        // Fake legend item is required for now -
        // for color axis only legend symbol is used.
        item.legendItem = item.chart.renderer.text('', 0, 0) as any;
