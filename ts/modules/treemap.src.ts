...
seriesType<Highcharts.TreemapSeries>(
    ...
    , {
        ...
        setPointValues: function (this: Highcharts.TreemapSeries): void {
            ...
            const translateX = (value: number): number => Math.round(
                xAxis.translate(value, false, false, false, true) as any
            );
            const translateY = (value: number): number => Math.round(
                // Flip y-values so that 0 is top of the axis
                yAxis.translate(100 - value, false, true, false, true) as any
            );
            ...
            points.forEach(function (point: Highcharts.TreemapPoint): void {
                ...
                if (values && visible) {
                    ...
                    const x1 = translateX(values.x) - crispCorr;
                    const x2 = translateX(values.x + values.width) - crispCorr;
                    const y1 = translateY(values.y) - crispCorr;
                    const y2 = translateY(values.y + values.height) - crispCorr;
