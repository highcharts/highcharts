...
SVGRenderer.prototype.symbols.arc = function (
    ...
    const alpha = end - start,
        ...
        // The angle that the radius of the small arc takes up in the big arc
        ...
        // The distance from the center of the big arc to the starting point of
        // the small arc
    ...
    // The main outer arc should stop where the next small circle touches the
    // great circle.
    const endOuter = end - angleOfBorderRadiusOuter;
    if (path[2] && path[2][0] === 'A') {
        // Long or short arc must be reconsidered because we have modified the
        // start and end points
        path[2][4] = pick(options.longArc, endOuter - startOuter < Math.PI) ?
            0 :
            1;
    ...
    // Draw an arc towards a point on the end angle, but one borderRadius closer
    // to the center relative to the perimeter.
    path.splice(3, 0, [
        'A',
        borderRadius,
        borderRadius,
    ...
    if (path[6] && path[6][0] === 'A') {
        // Long or short arc must be reconsidered because we have modified the
        // start and end points
        path[6][4] = pick(options.longArc, endInner - startInner < Math.PI) ?
            0 :
            1;
