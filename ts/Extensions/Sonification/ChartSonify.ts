...
function getTimeExtremes(
    ...
    return series.points.reduce(function (
        ...
        let value = getPointTimeValue(point, timeProp);
    ...
    return (allInstrumentDefinitions).reduce(function (
        ...
        Object.keys(instrumentDefinition.instrumentMapping || {}).forEach(
            function (instrumentParameter: string): void {
                let value = instrumentDefinition.instrumentMapping[
                    instrumentParameter
                ];
...
function makeInstrumentCopies(
    ...
    return instruments.map(function (
        ...
        let instrument = instrumentDef.instrument,
...
function buildTimelinePathFromSeries(
    ...
    let timeExtremes = options.timeExtremes || getTimeExtremes(series, options.pointPlayTime),
        ...
        timelineEvents = series.points.reduce(function (
            ...
            let earcons = getPointEarcons(point, options.earcons || []),
    ...
    return new H.sonification.TimelinePath({
        ...
        onEventStart: function (
            ...
            let eventObject = event.options && event.options.eventObject;
        ...
        onEventEnd: function (eventData: Highcharts.SignalDataObject): void {
            let eventObject = eventData.event && eventData.event.options &&
        ...
        order = (orderOptions as any).reduce(function (
            ...
            let simulItems: Array<(
                ...
                if (typeof item === 'string') {
                    let series: Highcharts.SonifyableSeries = (
...
function syncSimultaneousPaths(paths: Array<Highcharts.TimelinePath>): void {
    ...
    let extremes = paths.reduce(function (
    ...
    return order.reduce(function (
        ...
        return durationTotal + splat(orderDef).reduce(
            function (
                ...
                let timeExtremes: Highcharts.RangeObject = (
                    (item as any).series &&
...
function buildPathsFromOrder(
    ...
    // available duration.
    let totalAvailableDurationMs = Math.max(
    ...
    return order.reduce(function (
        ...
        let simultaneousPaths: Array<Highcharts.TimelinePath> =
            splat(orderDef).reduce(function (
...
function setCursor(
    ...
    let timeline: Highcharts.Timeline = this.sonification.timeline as any;
...
// Export functions
let chartSonifyFunctions: Highcharts.SonifyChartFunctionsObject = {
    chartSonify,
    seriesSonify,
    pause,
    resume,
