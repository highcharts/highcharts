...
let index: number | string, mainCanvas: HTMLCanvasElement | undefined;
  ...
  if (pushUnique(composed, "Boost.Series")) {
    ...
    (
      [
        ...
        "translate" | "generatePoints" | "drawTracker" | "drawPoints" | "render"
      >
    ).forEach((method): void =>
    ...
    if (wglMode) {
      ...
      if (BubbleSeries) {
        ...
        wrap(
          bubbleProto,
          "markerAttribs",
          function (this: typeof bubbleProto, proceed: Function): boolean {
            if (this.boosted) {
              return false;
            }
            return proceed.apply(this, [].slice.call(arguments, 1));
 ...
 * the canvas renderer
 */
function createAndAttachRenderer(chart: Chart, series: Series): WGLRenderer {
  ...
  if (isChartSeriesBoosting(chart)) {
    ...
    hasClickHandler = Boolean(
      series.options.events?.click || series.options.point?.events?.click,
    );
  }
  ...
  if (!boost.target) {
    ...
    if (chart.renderer.forExport || !foSupported) {
      ...
      target.renderTarget = boost.target = doc.createElement("canvas") as any;
    ...
    (boost.targetFo || boost.target).attr({
 ...
 */
function destroyGraphics(series: Series): void {
  const points = series.points;

  if (points) {
    let point: Point, i: number;
  ...
  (["graph", "area", "tracker"] as Array<"graph" | "area" | "tracker">).forEach(
    (prop): void => {
 ...
 */
function enterBoost(series: Series): void {
  ...
  (
    ["allowDG", "directTouch", "stickyTracking"] as Array<
  ...
  if (series.is("scatter") && !series.is("treemap") && series.data.length) {
 ...
 */
function exitBoost(series: Series): void {
  const boost = series.boost,
    chart = series.chart,
  ...
  if (chartBoost?.markerGroup) {
    ...
    for (const s of chart.series) {
      ...
      s.markerGroup = s
        .plotGroup("markerGroup", "markers", "visible", 1, chart.seriesGroup)
 ...
 */
function hasExtremes(series: Series, checkX?: boolean): boolean {
 ...
 * @internal
 */
function onSeriesDestroy(this: Series): void {
  const series = this,
    chart = series.chart;
...
/** @internal */
function onSeriesHide(this: Series): void {
...
function getPoint(
  ...
  const data = seriesOptions.data,
    ...
    xData =
      ...
      series.getColumn("x", true) ||
      false,
    yData = series.getColumn("y", true) || seriesOptions.yData || false,
    pointIndex = boostPoint.i,
    pointColor = (data?.[pointIndex] as { color?: string } | undefined)?.color,
    point = new PointClass(
      series as BoostSeriesComposition,
      isScatter && xData && yData
        ? [xData[pointIndex], yData[pointIndex]]
        : (isArray(data) ? data : [])[pointIndex],
      xData ? xData[pointIndex] : void 0,
    ) as BoostPointComposition;

  if (isScatter && seriesOptions?.keys?.length) {
    ...
    for (let keysIndex = keys.length - 1; keysIndex > -1; keysIndex--) {
      (point as any)[keys[keysIndex]] = (data as any)[pointIndex][keysIndex];
    }
  }
...
function scatterProcessData(
  ...
  if (!series.isDirty && !xAxis.isDirty && !yAxis.isDirty && !force) {
    return false;
  }
  ...
  // When data is not cropped (all data visible), update axis extremes from
  // the full data to ensure reset zoom restores correct axis ranges (#24386)
  if (!cropped) {
    if (Number.isFinite(xDataMax) && Number.isFinite(xDataMin)) {
      xAxis.dataMax = xDataMax;
      xAxis.dataMin = xDataMin;
    }
    if (Number.isFinite(yDataMax) && Number.isFinite(yDataMin)) {
      yAxis.dataMax = yDataMax;
      yAxis.dataMin = yDataMin;
    }
  }
...
function seriesRenderCanvas(this: Series): void {
  const options = this.options || {},
    ...
    isRange = this.pointArrayMap && this.pointArrayMap.join(",") === "low,high",
  ...
  if (tooltip && !tooltip.isHidden) {
    const isSeriesHovered =
      ...
      chart.hoverPoints?.some((point: Point): boolean => point.series === this);
  ...
  if (!isChartSeriesBoosting(chart)) {
    ...
    if (this.markerGroup && this.markerGroup !== chartBoost?.markerGroup) {
    ...
    if (seriesBoost && seriesBoost.target) {
      this.renderTarget = seriesBoost.target = seriesBoost.target.destroy();
    }
  }
      ...
      if (y !== null && x >= xMin && x <= xMax && isYInside) {
        ...
        if (sampling) {
          if (typeof minI === "undefined" || clientX === lastClientX) {
            ...
            if (typeof maxI === "undefined" || y > (maxVal as any)) {
              maxVal = y;
              maxI = i;
            }
            if (typeof minI === "undefined" || low < (minVal as any)) {
              minVal = low;
              minI = i;
            }
          }
          ...
          if (!compareX || clientX !== lastClientX) {
            ...
            if (typeof minI !== "undefined") {
              ...
              yBottom = yAxis.toPixels(minVal as any, true);
 ...
 * @internal
 */
function wrapSeriesDrawPoints(this: Series, proceed: Function): void {
  let enabled = true;
 ...
 * @internal
 */
function wrapSeriesProcessData(this: Series, proceed: Function): void {
  let dataToMeasure:
    | (PointOptions | PointShortOptions)[]
    | Types.TypedArray
    | undefined = this.options.data;
