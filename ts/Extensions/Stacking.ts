...
class StackItem {
    ...
    public setOffset(
        ...
        let stackItem = this,
            ...
            // x position:
            x =
                pick(defaultX, chart.xAxis[0].translate(stackItem.x) as any) +
                xOffset,
            stackBox =
                defined(y) &&
                stackItem.getStackBox(
                    chart,
                    stackItem,
                    x,
                    y as any,
    ...
    for (i = 0; i < yDataLength; i++) {
        ...
        if (stacking === 'percent') {
            ...
            if (negStacks && stacks[other as any] && stacks[other as any][x]) {
                ...
                stack.total = correctFloat(
                    (stack.total as any) + (Math.abs(y) || 0)
                );
            }
...
Series.prototype.modifyStacks = function (): void {
    ...
    if ((series as any)[stacking + 'Stacker']) {
