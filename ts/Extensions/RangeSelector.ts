...
class RangeSelector {
    ...
    public clickButton(i: number, redraw?: boolean): void {
        let rangeSelector = this,
            ...
            newMax =
                ...
                Math.round(
                    Math.min(
                        baseAxis.max as any,
                        pick(dataMax, baseAxis.max as any)
                    )
                ), // #1568
        ...
        if (type === 'month' || type === 'year') {
            ...
            if (baseAxis) {
                ...
                // point and extremes data (things like pointStart and
                // pointInterval are missing), so we delay the process (#942)
            } else {
            ...
            if (chart.navigator && chart.navigator.baseSeries[0]) {
                chart.navigator.baseSeries[0].xAxis.options.range =
                    void 0 as any;
            }
    ...
    public init(chart: Chart): void {
        ...
        this.eventsToUnbind.push(
            addEvent(chart, 'load', function (): void {
                // If a data grouping is applied to the current button, release
                // it when extremes change
    ...
    public updateButtonStates(): void {
        ...
        rangeSelector.buttonOptions.forEach(function (
            ...
            let range = rangeOptions._range,
                ...
                // the current view
                isTooGreatRange =
                    (range as any) > (dataMax as any) - (dataMin as any),
            ...
            if (
                ...
                actualRange + 36e5 >=
                    { month: 28, year: 365 }[type] * day * count -
                        offsetRange &&
                actualRange - 36e5 <=
    ...
    public showInput(name: 'min' | 'max'): void {
        ...
        if (input && dateBox && this.inputGroup) {
            ...
            if (isTextInput && inputBoxWidth) {
                ...
                // Inputs of types date, time or datetime-local should be
                // centered on top of the dateBox
            } else {
                css(input, {
                    left:
                        Math.min(
                    ...
                    top:
                        translateY -
                        (input.offsetHeight - dateBox.height) / 2 +
                        'px'
                });
            }
        }
        ...
        if (group && buttonPosition && inputPosition) {
            ...
            if (buttonGroup) {
                ...
                if (!this.initialButtonGroupWidth) {
                    ...
                    buttons.forEach((button: SVGElement, i: number): void => {
                        width += button.width;
        ...
        const groupsOverlap = (buttonGroupWidth: number): boolean => {
            if (inputGroup && buttonGroup) {
                ...
                return (
                    ...
                    buttonPosition.y <
                        inputPosition.y + inputGroup.getBBox().height
                );
            }
            return false;
        };
    ...
    public destroy(): void {
        ...
        if (rSelector.eventsToUnbind) {
            rSelector.eventsToUnbind.forEach((unbind: Function): void =>
                unbind()
            );
...
function preferredInputType(format: string): string {
    ...
    const date = ['a', 'A', 'd', 'e', 'w', 'b', 'B', 'm', 'o', 'y', 'Y'].some(
        (char: string): boolean => format.indexOf('%' + char) !== -1
    );
...
Axis.prototype.minFromRange = function (): number | undefined {
    let rangeOptions = this.range,
        ...
        getTrueRange = function (base: number, count: number): number {
            const timeName: Time.TimeUnitValue =
                type === 'year' ? 'FullYear' : 'Month';
