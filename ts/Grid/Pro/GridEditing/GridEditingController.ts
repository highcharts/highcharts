...
export class GridEditingController {
    ...
    public async deleteColumn(columnId: string): Promise<void> {
        ...
        if (needsAutogenOff) {
            this.disableAutogeneratedColumns();
        }
        grid.dirtyFlags.add('grid');
        await grid.update({ columns: nextColumns }, true, true);
    ...
    private disableAutogeneratedColumns(): void {
        const grid = this.grid;
        const userData = (grid.userOptions.data ?? {}) as DataProviderOptions;
        userData.autogenerateColumns = false;
        grid.userOptions.data = userData as typeof grid.userOptions.data;

        if (grid.options) {
            const data = (grid.options.data ?? {}) as DataProviderOptions;
            data.autogenerateColumns = false;
            grid.options.data = data as typeof grid.options.data;
        }
    }

    private syncLocalDataOptions(): void {
        const provider = this.grid.dataProvider;
        if (!GridEditingController.hasGetDataTable(provider)) {
            return;
        }

        const dataTable = provider.getDataTable(false);
        if (!dataTable) {
            return;
        }

        const sync = (options?: Partial<Options>): void => {
            if (!options) {
                return;
            }
            ...
            if ('columns' in data) {
                data.columns = dataTable.columns;
            }
        };

        sync(this.grid.userOptions);
        sync(this.grid.options);
    }
