...
class Tick {
    ...
    public getLabelPosition(
        ...
        if (!defined(yOffset)) {
            if (axis.side === 0) {
                ...
                // #3140, #3140
                yOffset =
                    Math.cos((label.rotation as any) * deg2rad) *
    ...
    public handleOverflow(xy: PositionObject): void {
        ...
        if (!rotation && labelOptions.overflow === 'justify') {
            ...
            if (modifiedSlotWidth < slotWidth && axis.labelAlign === 'center') {
                xy.x +=
                    ...
                    (slotWidth -
                        ...
                        xCorrection *
                            (slotWidth -
                                Math.min(labelWidth, modifiedSlotWidth)));
            }
            ...
            // Add ellipsis to prevent rotated labels to be clipped against the
            // edge of the chart
        } else if (
            (rotation as any) < 0 &&
    ...
    public renderMark(
        ...
        const tick = this,
            ...
            tickColor =
                options[type !== 'minor' ? 'tickColor' : 'minorTickColor'];
    ...
    public renderLabel(
        ...
        if (label && isNumber(x)) {
            ...
            // label anyway (#2100).
            if (
                (tick.isFirst && !tick.isLast && !options.showFirstLabel) ||
                (tick.isLast && !tick.isFirst && !options.showLastLabel)
            ) {
                show = false;
