...
const {
    seriesTypes: { area: AreaSeriesClass, line: LineSeriesClass }
} = SeriesRegistry;
...
wrap(
    ...
    'getGraphPath',
    function (this: AreaSeries, proceed: Function): SVGPath {
        const series = this,
        ...
        bottomPoints = perspective(bottomPoints as any, series.chart, true).map(
            function (point): AreaPoint {
                return {
                    plotX: point.x,
                    plotY: point.y,
                    plotZ: point.z
        ...
        if (series.areaPath) {
            ...
            areaPath = series.areaPath
                .splice(0, series.areaPath.length / 2)
                .concat(bottomPath);
