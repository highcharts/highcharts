...
class FlagsSeries extends ColumnSeries {
    ...
    public static defaultOptions: FlagsSeriesOptions = merge(
        ...
        {
            /**
             * In case the flag is placed on a series, on what point key to
             * place it. Line and columns have one key, `y`. In range or
             * OHLC-type series, however, the flag can optionally be placed on
             * the `open`, `high`, `low` or `close` key.
             *
             * @sample {highstock} stock/plotoptions/flags-onkey/ Range series,
             *         flag on high
             *
             * @type       {string}
             * @default    y
             ...
             *
             * @sample {highstock} stock/plotoptions/flags-allowoverlapx Allow
             *         sideways overlap
             *
             * @since 6.0.4
             */
            ...
            /**
             * When multiple flags in the same series fall on the same value,
             * this number determines the vertical offset between them.
             *
             * @sample {highstock} stock/plotoptions/flags-stackdistance/ A
             *         greater stack distance
             *
             * @product highstock
             */
            stackDistance: 12,
            ...
            /**
             * Specific tooltip options for flag series. Flag series tooltips
             * are different from most other types in that a flag doesn't have a
             * data value, so the tooltip rather displays the `text` option for
             * each point.
             *
             * @extends   plotOptions.series.tooltip
             * @excluding changeDecimals, valueDecimals, valuePrefix,
             * valueSuffix
             * @product   highstock
             */
            tooltip: {
            ...
            /**
             * The y position of the top left corner of the flag relative to
             * either the series (if onSeries is defined), or the x axis.
             * Defaults to `-30`.
             *
             * @product highstock
             */
            ...
            /**
             * Whether to use HTML to render the flag texts. Using HTML allows
             * for advanced formatting, images and reliable bi-directional text
             * rendering. Note that exported images won't respect the HTML, and
             * that HTML won't respect Z-index settings.
             *
             * @type      {boolean}
             * @default   false
             * @since     1.3
            ...
            /**
             * Fixed width of the flag's shape. By default, width is
             * autocalculated according to the flag's title.
             *
             * @sample {highstock} stock/demo/flags-shapes/ Flags with fixed
             *         width
             *
             * @type      {number}
             * @product   highstock
    ...
    public drawPoints(): void {
        ...
        if (options.useHTML) {
            wrap(
                series.markerGroup,
                'on',
                function (this: FlagsSeries, proceed): SVGElement {
        ...
        points.forEach(function (point): void {
            ...
            if (graphic) {
                ...
                point.unbindMouseOver = addEvent(
                    ...
                    function (): void {
                        // Raise this point
                        if ((point.stackIndex as any) > 0 && !point.raised) {
     ...
     * @private
     */
    public pointAttribs(point: FlagsPoint, state?: string): SVGAttributes {
