...
// Utils
function arrayExtremesOHLC(data: Array<Array<number>>): Record<string, number> {
...
class VBPIndicator extends SMAIndicator {
     ...
     */
    public static defaultOptions: VBPOptions = merge(
        SMAIndicator.defaultOptions,
        {
            ...
            /**
             * The styles for bars when volume is divided into
             * positive/negative.
             */
            volumeDivision: {
                /**
    ...
    public init(chart: Chart): VBPIndicator {
        let indicator = this,
    ...
    public posNegVolume(initVol: boolean, pos: boolean): void {
        let indicator = this,
            signOrder: Array<string> = pos
                ? ['positive', 'negative']
                : ['negative', 'positive'],
            volumeDivision: VBPIndicator.VBPIndicatorStyleOptions = indicator
                .options.volumeDivision as any,
    ...
    public translate(): void {
        let indicator = this,
            ...
            zoneLinesOptions: VBPIndicator.VBPIndicatorStyleOptions = indicator
                .options.zoneLines as any,
        ...
        if (indicatorPoints.length) {
            pointPadding =
                (options.pointPadding as any) < 0.5
            ...
            indicatorPoints.forEach(function (
                point: VBPPoint,
                index: number
            ): void {
                ...
                barY = point.plotY =
                    ...
                    chartPlotTop -
                    (yAxis.reversed ? barHeight - oldBarHeight : barHeight) -
                    yBarOffset;
                barWidth = correctFloat(
                    (primalBarWidth * priceZones[index].wholeVolumeData) /
                        maxVolume
                );
                point.pointWidth = barWidth;

                // eslint-disable-next-line no-useless-call
                point.shapeArgs = indicator.crispCol.apply(indicator, [
                    barX,
                    barY,
                    barWidth,
                    barHeight
    ...
    public specifyZones(
        ...
        let indicator = this,
            rangeExtremes: boolean | Record<string, number> = isOHLC
                ? arrayExtremesOHLC(yValues)
                : false,
    ...
    public volumePerZone(
        ...
        let indicator = this,
            ...
            volumeYData: Array<number> = volumeSeries.processedYData as any,
        ...
        priceZones.forEach(function (
            ...
            for (i = 0; i < baseSeriesLength; i++) {
                ...
                if (!indicator.options.compareToMain && mainSeries.dataModify) {
                    ...
                    previousValue =
                        mainSeries.dataModify.modifyValue(previousValue);
                }
    ...
    public drawZones(
        ...
        zonesValues.forEach(function (value: number): void {
            ...
            zoneLinesPath = zoneLinesPath.concat(
                chart.renderer.crispLine(
                    [
                        ['M', leftLinePos, verticalLinePos],
                        ['L', rightLinePos, verticalLinePos]
                    ],
        ...
        if (zoneLinesSVG) {
            ...
            zoneLinesSVG = indicator.zoneLinesSVG = renderer
                .path(zoneLinesPath)
                .attr({
