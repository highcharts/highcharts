...
class Toolbar {
    ...
    public cachedIcons: Record<string, string>;
    public width = 0;
    ...
    public addButton(
        ...
        mainButton.style.backgroundImage = this.getIcon(
            btnOptions.symbol as string
        );
    ...
    public addNavigation(): void {
        ...
        this.arrowUp.style.backgroundImage = this.getIcon('arrow-right.svg');

        this.arrowDown = createElement('div', {
            className: 'highcharts-arrow-down'
        }, void 0, this.arrowWrapper);

        this.arrowDown.style.backgroundImage = this.getIcon('arrow-right.svg');
    ...
    public showHideToolbar(): void {
        ...
        showHideBtn.style.backgroundImage = this.getIcon('arrow-right.svg');
     ...
     * our servers. Since v12.x, icons are generated by JS in SVG, then used
     * as base64.
     *
     * @private
     */
    getIcon(iconName: string): string {
        ...
        const fallbackURL = 'https://code.highcharts.com/gfx/stock-icons/';


        if (iconsURL === 'renderer') {
            let base64Icon = this.cachedIcons[iconName];

            if (!base64Icon) {

                // Not translated, fallback to server icon:
                if (!(this.chart.renderer.symbols as any)[iconName]) {
                    return `url("${fallbackURL}${iconName}")`;
                }

                const svgIcon = this.chart.renderer
                    .path((this.chart.renderer.symbols as any)[iconName]())
                    .attr({
                        stroke: 'rgb(75, 75, 75)',
                        'stroke-width': 1
                    })
                ...
                const serialized = new XMLSerializer().serializeToString(
                    svgIcon.element as Node
                );
                const wrappedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">${serialized}</svg>`;
                this.cachedIcons[iconName] = base64Icon = btoa(
