...
- DOM: `makeHTMLElement` in `GridUtils.ts`. Class names come from `Globals.getClassName(key)` (prefix `hcg-`; add keys to `rawClassNames`). Style them in `css/grid/`: Core styles in `grid-lite.css` itself, which imports only the `*-variables.css` files and the theme; Pro-only styles in a per-feature file that `grid-pro.css` imports. There is no styled mode.
- CSS has three layers, and a rule belongs to exactly one of them:
  1. Tokens. `--hcg-*` is the public option a theme or a user sets; `--ig-*` resolves it, as `var(--hcg-x, <neutral fallback>)`, in the `*-variables.css` files. Resolution is unconditional, on `.hcg-container`, so the token exists with or without a theme. Neutral fallbacks are geometry and scale (`2px`, `100%`, `1em`), never a look.
  2. Application. What decides the layer is not the property but what the declaration settles: how the grid works (display, flex, position, widths, overflow) or how it looks. Functional rules apply unconditionally, with plain values: Core's in `grid-lite.css`, Pro's in `grid-pro.css` and the feature files it imports. Every rule that settles a look goes inside a `.hcg-themed` block, because `hcg-themed` lands on the container only when `rendering.theme` is set, and `theme: ''` has to render a grid with no opinions at all. Both directions are absolute and both are greppable: inside `.hcg-themed` no declaration carries a literal, every value is a `var(--ig-*)`; outside it no declaration reads an `--ig-*` token for a look, not even with a fallback. A paint token outside a themed block means the rule sits in the wrong layer, not that it needs a default.
  3. Theme. `grid-theme-default.css` holds only `--hcg-*` tokens, no rules, in `.highcharts-light`, `.highcharts-dark` and `prefers-color-scheme` variants. Colors live here and nowhere else.
- Two consequences that are easy to get wrong. Application rules use `:where(.hcg-x)` so that a single user class beats them; a feature rule written as `.hcg-container .hcg-x` therefore overrides the themed surface instead of extending it, and silently opts that element out of every theme. And a component variant (the same button in a cell, in pagination) redeclares the `--ig-*` tokens in a narrower selector over an `--ig-*-base` default; it does not add a second application rule.
- Files written before this layering still break it, so don't take them as the pattern: the loader in `grid-lite.css`, cell editing, notifications and Pro credits in `grid-pro.css`, and the sticky group row in `grid-tree-view.css` all read paint tokens outside `.hcg-themed`. `grid-table-variables.css` and `grid-button-variables.css` are the ones to copy.
