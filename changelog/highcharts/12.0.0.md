...
## Upgrade notes
- When importing Highcharts as a node module, additional modules no longer requires initialization. Previously modules would be first imported, then initialized:
```js
...
```
Since v12, the module will not return a factory function, so this will cause an error. Instead, import it like this:
```js
